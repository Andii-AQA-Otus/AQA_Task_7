...
class LogsParser:
    ...
    def __init__(self, logs_source, json_file):
        ...
        self.LOG_LINE_PATTERN = re.compile(
            pattern=r'((?:\d{1,3}\.){3}\d{1,3}|(?:[a-f0-9]{1,4}:){7}[a-f0-9]{1,4}|::\d)\s+-\s+-\s+\[.*?\+\d+\]\s+"('
                    r'GET|HEAD|POST|PUT|DELETE|OPTIONS)\s+(\/?.*?)\s+HTTP\/1\.\d"\s+(\d+)\s+(\d+)',
            flags=re.IGNORECASE
        )
    ...
    def save_report(self):
        with open(file=str(self.json_file), mode="w") as f:
            data = {
                "requests total": self.get_total_number_of_requests(),
                "requests by type": self.get_requests_by_type(),
                "top 10 ip addresses": self.get_top_10_ip_addresses(),
                "top 10 longest requests": self.get_top_10_longest_requests(),
                "top 10 client error requests": self.get_top_10_client_error_requests(),
                "top 10 server error requests": self.get_top_10_server_error_requests()
            }
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(prog="log_parser")
    arg_parser.add_argument("--input", action="store", help="Path to directory with log files or path to specific log "
                                                            "file")
